...
printf "[1/4] 메인 티커 (예: TQQQ, QLD): "
read -r TICKER_MAIN
TICKER_MAIN=$(echo "$TICKER_MAIN" | tr '[:lower:]' '[:upper:]' | xargs)

printf "[2/4] 기준선이 될 티커 (예: TQQQ, QLD): "
read -r TICKER_LINE
TICKER_LINE=$(echo "$TICKER_LINE" | tr '[:lower:]' '[:upper:]' | xargs)

printf "[3/4] 안전 자산 코드 (예: SGOV, USFR): "
read -r TICKER_RISK_OFF
TICKER_RISK_OFF=$(echo "$TICKER_RISK_OFF" | tr '[:lower:]' '[:upper:]' | xargs)

printf "[4/4] 추가 매수 자산 코드 (예: SPYM, VOO): "
...
print(f"장 마감. 16:04부터 {TICKER_LINE} 데이터 폴링 시작...")
...
for retries in range(MAX_RETRIES):
    try:
        resp = session.get(
            f"https://query1.finance.yahoo.com/v8/finance/chart/{TICKER_LINE}",
            params={
...
echo "    즉시 테스트 알림 전송 중...    "
echo "=================================="
echo "※ 지금 시각 기준으로 Yahoo Finance에서 데이터를 가져와"
echo "   오늘 전략을 판단하고 텔레그램으로 전송합니다."
echo ""

# 바로 실행 (오류 무시: 장 중이라면 데이터 없을 수 있음)
python "$HOME/$TICKER_MAIN.py" 2>/dev/null || \
    echo "경고: 테스트 실행 실패 (장 중이거나 네트워크 문제일 수 있음)"

echo ""
echo "테스트 완료! 텔레그램 확인해보세요."
echo "※ 장 마감 전이라면 '데이터 없음' 또는 '대기' 메시지가 올 수 있습니다."
